...
import {
  ...
  type AtaRpc,
} from "../lib/phantom-ata-session.js";
...
const RECOVERY_PATH = resolve(".avicoin-phantom-sessions/create-ata-recovery.json");
const MAX_REQUEST_BYTES = 1_000_000;

class FileAtaRecoveryStore implements AtaRecoveryStore {
  constructor(private readonly path = RECOVERY_PATH) {}
  async load(): Promise<AtaRecoveryRecord | null> {
    try { return JSON.parse(await readFile(this.path, "utf8")) as AtaRecoveryRecord; }
    catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return null; throw error; }
  }
  async save(record: AtaRecoveryRecord): Promise<void> {
...
export function ataRuntimeAuthorization(environment: NodeJS.ProcessEnv = process.env): PhantomRuntimeAuthorization {
  return {
...
class Web3AtaRpc implements AtaRpc {
  ...
  async getFeeForMessage(message: VersionedMessage) {
    ...
    if (fee.value === null) throw new Error("El RPC no pudo estimar el fee ATA.");
  ...
  async simulate(transaction: VersionedTransaction) {
    ...
    if (result.value.err) throw new Error(`Simulación ATA falló: ${JSON.stringify(result.value.err)}`);
  ...
  async confirmFinalized(input: { readonly signature: string; readonly blockhash: string; readonly lastValidBlockHeight: number }) {
    ...
    if (result.value.err) throw new Error(`ATA finalizó con error: ${JSON.stringify(result.value.err)}`);
  }
  ...
  async readAta(address: PublicKey) {
    ...
    return { programOwner: accountInfo.owner.toBase58(), mint: account.mint.toBase58(), owner: account.owner.toBase58(), amount: account.amount };
  }
}
...
export async function startAtaPhantomServer(options: { readonly port?: number; readonly environment?: NodeJS.ProcessEnv } = {}) {
  const environment = options.environment ?? process.env;
  const port = options.port ?? Number(environment.AVICOIN_PHANTOM_PORT ?? "4175");
  if (!Number.isInteger(port) || port < 1024 || port > 65_535) throw new Error("Puerto local inválido.");
  const runtime = () => ataRuntimeAuthorization(environment);
  const connection = new Connection(runtime().rpcUrl, "confirmed");
  const rpc = new Web3AtaRpc(connection);
  const recovery = new FileAtaRecoveryStore();
  const assertExpectedState = async () => {
    const state = await loadMainnetState();
    assertStateAllows(state, "create-ata");
    if (state.avi_mint !== AVICOIN_MAINNET_MINT || !state.metadata_created || state.metadata_pda !== "4jJmQbSYi3k1iunsbC6qcJM477T8apTw1SoyY36j1Qp2" || state.ata_created || state.avi_ata) throw new Error("Estado local no permite create-ata.");
    if (state.supply_minted || state.pool_created || state.mint_authority_revoked) throw new Error("Estado local posterior fuera de alcance.");
  };
  const assertMetadata = async () => {
    const pda = await assertMainnetMetadata(runtime().rpcUrl, AVICOIN_MAINNET_MINT, MAINNET_METADATA_URI);
    if (pda !== "4jJmQbSYi3k1iunsbC6qcJM477T8apTw1SoyY36j1Qp2") throw new Error("Metadata PDA inesperada.");
  };
  const coordinator = new PhantomCreateAtaCoordinator(rpc, runtime, recovery, assertExpectedState, assertMetadata, async (ata) => {
    const state = await loadMainnetState();
    if (state.ata_created || state.avi_ata) throw new Error("Estado ATA ya fue actualizado.");
    await writeMainnetState({ ...state, avi_ata: ata, ata_created: true });
  });
  const bundleDirectory = await mkdtemp(join(tmpdir(), "avicoin-phantom-ata-ui-"));
  const bundlePath = join(bundleDirectory, "app.js");
  await bundle({ entryPoints: [resolve("tools/phantom-ata/app.js")], outfile: bundlePath, bundle: true, platform: "browser", format: "esm", target: ["es2022"], minify: false, sourcemap: false, logLevel: "silent" });
  const assets = new Map([
    ["/", { path: resolve("tools/phantom-ata/index.html"), type: "text/html; charset=utf-8" }],
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/bootstrap") {
        ...
        json(response, 200, {
          ...
          productionWallet: auth.productionWallet, mint: AVICOIN_MAINNET_MINT, ata: AVICOIN_MAINNET_ATA,
          selectedOperation: auth.operation ?? null,
          executionEnabled: auth.allowMainnet && auth.operation === "create-ata" && auth.confirmationToken === CREATE_ATA_CONFIRMATION_TOKEN,
          recovery: await recovery.load(),
          preflight: {
            ataStillAbsent: (await rpc.readAta(new PublicKey(AVICOIN_MAINNET_ATA))) === null && state.avi_ata === null && !state.ata_created,
            mintInvariantsValid: mint.owner === TOKEN_PROGRAM_ID.toBase58() && mint.snapshot.decimals === 9 && mint.snapshot.supply === 0n && mint.snapshot.mintAuthority === MAINNET_PRODUCTION_WALLET && mint.snapshot.freezeAuthority === null,
            metadataValid,
          },
        }); return;
      }
  ...
  return { server, port, close: async () => { await new Promise<void>((resolveClose, reject) => server.close((error) => error ? reject(error) : resolveClose())); await rm(bundleDirectory, { recursive: true, force: true }); } };
}

export async function main(): Promise<void> {
  const instance = await startAtaPhantomServer();
  ...
  console.log(`AVICOIN Phantom ATA: http://127.0.0.1:${instance.port}/`);
  console.log("No se solicita firma al iniciar. Única operación: create-ata. mintTo está bloqueado.");
}
