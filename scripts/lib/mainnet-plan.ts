...
export function buildMainnetUnsignedPlan(input: {
  ...
  const plans = [
    ...
    operationPlan({ order: 2, operation: "create-metadata", kind: "transaction", instructions: ["compute-budget:setComputeUnitLimit(200000)", "compute-budget:setComputeUnitPrice(1000 micro-lamports)", "token-metadata:createMetadataAccountV3(mutable, sellerFee=0)"], programs: ["ComputeBudget111111111111111111111111111111", metadata], writableAccounts: [wallet, mint, "metadata_pda"], requiredSigners: [wallet], dependsOn: ["create-mint finalized and re-read"], stopConditions: ["metadata PDA exists", "public metadata hash mismatch", "signed message differs from simulated message"] }),
