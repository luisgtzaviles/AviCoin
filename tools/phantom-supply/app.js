...
export const ENABLED_OPERATION = "mint-fixed-supply";
...
export const EXPECTED_CONFIRMATION_TOKEN = "CONFIRMO-MAINNET-EMITIR-1000-AVI";
export const EXPECTED_AMOUNT_AVI = "1000";
export const EXPECTED_BASE_UNITS = "1000000000000";
...
export async function connectForSupply(provider) {
...
export function initializeSupplyPage(documentRef = document, provider = window.phantom?.solana) {
  ...
  const refresh = () => {
    ...
    const checks = {
      ...
      supplyZero: state.bootstrap?.preflight?.supplyZero === true,
      ataZero: state.bootstrap?.preflight?.ataZero === true,
      ...
      operationEqualsMintFixedSupply: state.bootstrap?.selectedOperation === ENABLED_OPERATION,
    };
  ...
  elements.connect.addEventListener("click", () => run(async () => {
    state.wallet = await connectForSupply(provider);
    state.bootstrap = await api("/api/bootstrap");
    if (state.bootstrap.productionWallet !== state.wallet || state.bootstrap.mint !== MINT_ADDRESS || state.bootstrap.ata !== ATA_ADDRESS || state.bootstrap.amountAvi !== EXPECTED_AMOUNT_AVI || state.bootstrap.amountBaseUnits !== EXPECTED_BASE_UNITS) throw new Error("Bootstrap no coincide con wallet, mint, ATA o cantidad fija.");
    state.serverSession = await api("/api/session-status");
    elements.gate.textContent = state.bootstrap.executionEnabled ? "Autorización temporal activa sólo para mint-fixed-supply de 1,000 AVI." : "ALLOW_MAINNET=false: firma y Send bloqueados.";
    setStatus(`Wallet verificada: ${state.wallet}. No se solicitó firma.`, "success");
  }));
  elements.build.addEventListener("click", () => run(async () => { walletStillMatches(); accept(await api("/api/build", { connectedWallet: state.wallet, operation: ENABLED_OPERATION }), "Plan estable creado para exactamente 1,000 AVI; sin blockhash final."); }));
  elements.review.addEventListener("click", () => run(async () => { walletStillMatches(); accept(await api("/api/review", common()), "Plan revisado. Confirma mint, ATA, supply 0 y cantidad fija."); }));
  ...
  elements.requestSignature.addEventListener("click", () => run(async () => {
    ...
    try {
      ...
      accept(await api("/api/signed", { ...common(), messageHash: payload.messageHash, transactionBase64: toBase64(signed.serialize()) }), "Phantom firmó exactamente la emisión fija. Confirma el envío único.");
    } catch (error) {
  ...
  elements.verify.addEventListener("click", () => run(async () => {
    ...
    accept(session, session.status === "finalized" ? "Finalized PASS: supply y ATA contienen exactamente 1,000 AVI." : "Resultado ambiguo: no reintentes.", session.status === "finalized" ? "success" : "error");
  }));
...
if (typeof window !== "undefined" && typeof document !== "undefined") initializeSupplyPage();
