...
export const ENABLED_OPERATION = "mint-final-supply";
...
export const EXPECTED_CONFIRMATION_TOKEN = "CONFIRMO-MAINNET-EMITIR-SUPPLY-DEFINITIVO";
export const EXPECTED_AMOUNT_AVI = "99999000";
export const EXPECTED_BASE_UNITS = "99999000000000000";
export const EXPECTED_CURRENT_SUPPLY_BASE_UNITS = "1000000000000";
export const EXPECTED_FINAL_SUPPLY_AVI = "100000000";
export const EXPECTED_FINAL_SUPPLY_BASE_UNITS = "100000000000000000";
export const BLOCKED_OPERATIONS = ["create-mint", "create-ATA", "create-metadata", "revoke-mint-authority", "create-pool", "open-position", "liquidity", "swaps", "Devnet"];
...
export async function connectForFinalSupply(provider) {
...
export function initializeFinalSupplyPage(documentRef = document, provider = window.phantom?.solana) {
  ...
  const refresh = () => {
    ...
    const checks = {
      ...
      currentSupplyExact: state.bootstrap?.preflight?.currentSupplyExact === true,
      ataBalanceExact: state.bootstrap?.preflight?.ataBalanceExact === true,
      onlyOfficialAta: state.bootstrap?.preflight?.onlyOfficialAta === true,
      ...
      operationEqualsMintFinalSupply: state.bootstrap?.selectedOperation === ENABLED_OPERATION,
    };
    ...
    elements.signatureValidity.textContent = canSign ? "Request signature habilitado para el único mensaje simulado." : "Firma bloqueada hasta estado simulated autoritativo y margen válido.";
  };
  const accept = (session, message, tone = "success") => {
    state.session = session;
    state.serverSession = { sessionId: session.sessionId, status: session.status, planHash: session.plan.planHash, planReviewed: session.planReviewed };
    renderPlan(session.plan);
    renderFresh(session.freshTransaction);
  ...
  const run = async (work) => {
    ...
    refresh();
    try { await work(); }
    catch (error) { setStatus(error instanceof Error ? error.message : "Error local.", "error"); }
    finally { state.busy = false; refresh(); }
  };

  elements.connect.addEventListener("click", () => run(async () => {
    state.wallet = await connectForFinalSupply(provider);
    state.bootstrap = await api("/api/bootstrap");
    if (
      state.bootstrap.productionWallet !== state.wallet
      || state.bootstrap.mint !== MINT_ADDRESS
      || state.bootstrap.ata !== ATA_ADDRESS
      || state.bootstrap.amountAvi !== EXPECTED_AMOUNT_AVI
      || state.bootstrap.amountBaseUnits !== EXPECTED_BASE_UNITS
      || state.bootstrap.currentSupplyBaseUnits !== EXPECTED_CURRENT_SUPPLY_BASE_UNITS
      || state.bootstrap.finalSupplyAvi !== EXPECTED_FINAL_SUPPLY_AVI
      || state.bootstrap.finalSupplyBaseUnits !== EXPECTED_FINAL_SUPPLY_BASE_UNITS
    ) throw new Error("Bootstrap no coincide con wallet, mint, ATA, supply previo, emisión o supply final.");
    state.serverSession = await api("/api/session-status");
    elements.gate.textContent = state.bootstrap.executionEnabled ? "Autorización temporal activa sólo para mint-final-supply de 99,999,000 AVI." : "ALLOW_MAINNET=false: firma y Send bloqueados.";
    setStatus(`Wallet verificada: ${state.wallet}. No se solicitó firma.`, "success");
  }));
  elements.build.addEventListener("click", () => run(async () => { walletStillMatches(); accept(await api("/api/build", { connectedWallet: state.wallet, operation: ENABLED_OPERATION }), "Plan estable creado para exactamente 99,999,000 AVI; sin blockhash final."); }));
  elements.review.addEventListener("click", () => run(async () => { walletStillMatches(); accept(await api("/api/review", common()), "Plan revisado. Confirma supply previo 1,000 AVI, mint, ATA y cantidad fija."); }));
  elements.prepare.addEventListener("click", () => run(async () => { walletStillMatches(); accept(await api("/api/prepare", { ...common(), confirmationToken: elements.token.value, explicitlyConfirmed: elements.firstConfirm.checked }), "Mensaje fresco mintToChecked simulado. Firma requerida ahora."); }));
  elements.requestSignature.addEventListener("click", () => run(async () => {
    walletStillMatches();
    const payload = await api("/api/signing-payload", { ...common(), confirmationToken: elements.token.value, explicitlyConfirmed: elements.firstConfirm.checked });
    const transaction = VersionedTransaction.deserialize(fromBase64(payload.transactionBase64));
    try {
      const signed = await provider.signTransaction(transaction);
      accept(await api("/api/signed", { ...common(), messageHash: payload.messageHash, transactionBase64: toBase64(signed.serialize()) }), "Phantom firmó exactamente la emisión final declarada. Confirma el envío único.");
    } catch (error) {
      try { accept(await api("/api/signature-aborted", common()), "Phantom no devolvió firma; solicitud revertida.", "error"); } catch {}
      throw error;
    }
  }));
  elements.send.addEventListener("click", () => {
    ...
    return run(async () => { walletStillMatches(); accept(await api("/api/send", { ...common(), confirmationToken, explicitlyConfirmed }), "Send único realizado. Ejecuta Verify finalized."); });
  });
  elements.verify.addEventListener("click", () => run(async () => {
    walletStillMatches();
    const session = await api("/api/verify", common());
    accept(session, session.status === "finalized" ? "Finalized PASS: supply y ATA contienen exactamente 100,000,000 AVI." : "Resultado ambiguo: no reintentes.", session.status === "finalized" ? "success" : "error");
  }));
  elements.cancel.addEventListener("click", () => run(async () => { walletStillMatches(); accept(await api("/api/cancel", common()), "Sesión de emisión cancelada."); }));
  for (const event of ["input", "change"]) {
    elements.token.addEventListener(event, refresh);
    elements.firstConfirm.addEventListener(event, refresh);
    elements.sendConfirm.addEventListener(event, refresh);
  }
  ...
  window.setInterval(() => {
    ...
    void api("/api/fresh-status", common()).then((session) => accept(session)).catch((error) => setStatus(error instanceof Error ? error.message : "No se actualizó vigencia.", "error"));
  }, 5_000);
  refresh();
  return { state, refresh };
}

if (typeof window !== "undefined" && typeof document !== "undefined") initializeFinalSupplyPage();
